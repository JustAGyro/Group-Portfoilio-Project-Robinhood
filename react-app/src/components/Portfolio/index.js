...
export default function Portfolio() {
    ...
    const createHistory = (transactionHistory) => {
        ...
        let orderedHistory = groupBy(transactionHistory, ['date', 'symbol','transaction'])
        ...
        for(let date = new Date(firstTransactionDate); date < currentDate; date.setDate(date.getDate() + 1)){
            ...
            if(data[date.toDateString()]){
                ...
                keys.forEach(d => {
                  ...
                  data[date.toDateString()][d].buy?.forEach(q => {
                    ...
                    returnData[date.toDateString()][d] ? returnData[date.toDateString()][d] += q.quantity : returnData[date.toDateString()][d] = q.quantity;
                  })
            ...
            returnData[newDate.toDateString()] = {...returnData[date.toDateString()]};
        }
        // console.log("FLAG",returnData)
        let dataEntries = Object.entries(returnData);
        let sortedDataEntries = dataEntries.sort(function(a,b){
